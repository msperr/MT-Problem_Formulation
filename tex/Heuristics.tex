...
\begin{figure}[htb]
	...
	\end{tikzpicture}
	\caption{Vehicle cost for $(\operatorname{SMILP}_i)$}
...
\Cref{fig:graph_instance_1} shows the vehicle cost for partial instance~$I_1$.
...
\begin{figure}[htb]
	...
	\begin{tikzpicture}[node distance=1cm, auto,]
		...
		\draw[arrow] (vehicles) to node[auto] {$c^{\operatorname{v}}$} (trips.west);
		\draw[arrow] (vehicles) to node[auto] {$c^{\operatorname{v}}$} (endpoints.west);
		\draw[arrow] (trips) to node[auto] {$0$} (endpoints);
		\draw[arrow] (trips.east) to node[auto] {$0$} (de);
		\draw[arrow] (endpoints.east) to node[auto] {$-c^{\operatorname{v}}$} (de);
	\end{tikzpicture}
	\caption{Vehicle cost for $(\operatorname{SMILP}_1)$}
