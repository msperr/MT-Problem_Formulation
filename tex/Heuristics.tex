...
and
\begin{align}
	f^0_{v'}\geq e_{t_{a+1}} + \fd_{v',t_{a+1}}.\label{eq:doubleduties:fuel}
\end{align}
...
Let additionally
\begin{align}
	\cd_{v',t_{a+1}}\leq \cd_{v,t_1} + \sum_{j=1}^a \left(\ct_j + \cd_{j,j+1}\right) \label{eq:doubleduties:cost}
\end{align}
...
This statement also holds if there is a ${r\in\mathcal{R}_{v',t_{a+1}}}$ such that
\begin{align*}
	f^0_{v'}\geq e_{t_{a+1}} + \left(\fd_{v',r}+\ft_r+\fd_{r,t_{a+1}}\right) &&	\cd_{v',r}+\cd_{r,t_{a+1}}\leq \cd_{v,t_1} + \sum_{j=1}^a \left(\ct_j + \cd_{j,j+1}\right)
\end{align*}
