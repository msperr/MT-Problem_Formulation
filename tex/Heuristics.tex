...
This heuristic is directly applied from \cite[Sec. 10.4]{Knoll}. As presented there, the partial instances can be arranged arbitrarily. They argue that the setting with ordering from the end to the beginning suits best to the underlying instance structure. Therefore present only this approach here.
...
Every end point $t\in\Phat_i$ for $i\in[n]$ has to be covered by some duty out of $S_i$. This ensures that each all duties of the partial solutions are finally part of the overall solution. \Cref{alg:successive_heuristic} describes the procedure of the Successive Heuristics.

\begin{algorithm}[htb]
	\SetAlgoLined
	\KwIn{splitting $\mathcal{T}=\left\{\mathcal{T}_1,\dots,\mathcal{T}_n\right\}, \mathcal{V}$}
	\KwOut{overall solution $S$}
	\lForEach{$i\in[n]$}{$\Phat_i\gets\emptyset$}
	\ForEach{$i=n-1,\dots,2$}{
	...
	\ForEach{$d\in D_1$}{
		\While{$\operatorname{end}(d)\in\bigcup_{i=1}^{n}\Phat_i$}{
			determine duty $d'=\left(t_1,\dots,t_{l'}\right)$ with $t_1$ representing $t$\;
			delete $\operatorname{end}(d)$ from $d$\;
			append $d'$ to $d$\;
		}
	}
	\Return{$S$ with duties $D_1$}
	\caption{Successive Heuristic (general setting) \label{alg:successive_heuristic}}
\end{algorithm}\fxnote{notation of end(d)}
