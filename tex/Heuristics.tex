...
We define the additional values:
\begin{align*}
...
After defining an indicator for bad route choices, we decide which of the customers we want to review. It is presumably not profitable to solve a subproblem for a single customer. Instead we create a set ${A\subseteq\mathcal{C}}$ with customers that are reviewed. We aim this procedure to be efficient. To achieve this, the subproblem should significantly decrease the objective value of the solution on the one hand, on the other hand it should be solved very fast. We examine desirable properties of the customer set~$A$.
...
We have to consider these properties for creating the customer set~$A$. Further we have to keep the subproblem small, such that it can be solved in reasonable time. Therefore we make additional assumptions on~$A$:
\begin{enumerate}
\setcounter{enumi}{4}
	\item The time interval ${[\max_{c\in A}\zend_c, \min_{c\in A}\zstart_c]}$ is small. \\
	In the subproblem, all duties stay fixed up to this time interval. A small time interval causes a small number of trips that are variable \wrt the duties. This kepps the size of the subproblem small.
	\item The set ${(M\circ C)^{-1}(A)}$ is small. \\
	This set contains all trips of one of the customers in~$A$. In the subproblem, all other trips are fixed to be fulfilled. Therefore a small number of trips that are variable \wrt the duties or can be neglected keeps the subproblem small.
\end{enumerate}

It is not possible to meet the requirements for~$A$ and simultaneously cover all customers with potential bad route choices. Therefore we partition a into several subsets ${A=\Cupdot_{j=1}^n A_j}$. It suffices if $A_j$ fulfill the set requirements for all ${j\in[k]}$. We iteratively execute the subproblem, each with a small customer set~$A_j$.

\begin{algorithm}[htb]
	\SetAlgoLined
	\KwIn{solution $S$, $\mathcal{C}$, $\bar{m}_c$ for ${c\in\mathcal{C}}$, $r_{\min}, c_{\max}, t_{\max}, n_{\max}$}
	\KwOut{$\left\{A_1,\dots,A_k\right\}$}
	\lForEach{$c\in\mathcal{C}$}{determine $C_2(S,c)$}
	$A\gets\left\{c\in\mathcal{C}\mid\frac{C_2(S,c)}{C_1\left(\bar{m}_c\right)}\geq r_{\min}, \exists m\in C^{-1}(c)\backslash\left\{\bar{m}_c\right\} \text{ with } C_1(m)<C_2(S,c)\right\}$\;
	$i\gets 1$\;
	\While{$A\neq\emptyset\land i\leq n_{\max}$}{
		$\bar{c}\gets\argmax_{c\in A}\left(\frac{C_2(S,c)}{C_1\left(\bar{m}_c\right)}\right)$\;
		$A_i\gets\left\{c\in A\mid \zstart_{\bar{c}} - \frac{t_{\max}}{2} \leq \zstart_c, \zend_c \leq \zstart_{\bar{c}} + \frac{t_{\max}}{2}\right\}$\;
		\lWhile{$|A_i| > c_{\max}$}{$A_i\gets A_i\backslash\left\{\argmin_{c\in A_i}\left(\frac{C_2(S,c)}{C_1\left(\bar{m}_c\right)}\right)\right\}$}
		\lIf{$A_i=\emptyset$}{$A\gets A\backslash\left\{\bar{c}\right\}$}
		\lElse{$A\gets A\backslash A_i$; $i\gets i+1$}
	}
	$k\gets i$\;
	\Return{$\left\{A_1,\dots,A_k\right\}$}
...
\begin{itemize}
	\item{$r_{\min}$:} The minimal ratio $\frac{C_2(S,c)}{C_1\left(\bar{m}\right)}$ for ${c\in A}$
	\item{$c_{\max}$:} The maximal number of customers in $A_j$
	\item{$t_{\max}$:} The maximal time range of the customers in $A_j$
	\item{$n_{\max}$:} The maximal number of iterations
\end{itemize}

No guarantee. $A_1=\mathcal{C}$ guarantees optimality but too big. $A=\mathcal{C}$ does not ensure optimality. There can be route choices that are bad in $S^*$ but optimal in current $S$. Not necessarily all properties apply. $C_2$ is only estimated. Subproblem cannot fix all other bad heuristical decisions.
