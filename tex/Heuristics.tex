...
Let $S$ be a solution that is created with \Cref{alg:successive_heuristic} and the respective partial solutions $S_i$ are feasible in $(\operatorname{SMILP}_i)$ for ${i\in[n]}$. Then $S$ can be converted into a solution $S'$ which is feasible in the $\eqref{eq:SMILP}$ without the cover constraints $\eqref{eq:MMILP:customer}$ and $\eqref{eq:MMILP:route}$. The objective values of $S$ and $S'$ coincide without respecting the route costs.
...
Strictly speaking, $\Vhat_2$ and $\Phat_2$ consists of start or end points that are created from the respective trips. For each start or end point~$t$ that is created from a trip or vehicle $s$ hold the following properties:
\begin{align*}
