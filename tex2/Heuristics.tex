...
As mentioned before, it requires some additional work to include the cover constraints into the partial instances. The fulfilling of the cover constraints is also part of the respective heuristics and therefore $\eqref{eq:MMILP:costumer}$ and $\eqref{eq:MMILP:route}$ are left out in this formulation.
...
%########################################################################################################################################
%#
%#   Costumer-dependent Splitting
%#
%########################################################################################################################################

\section{Costumer-dependent Splitting}
\label{sec:costumer_dependent_splitting}

In this section, we introduce costumer-dependent splitting. In contrast to the splitting performed by \cite{Knoll}, the trips are not split according to their start times but according to their costumers' start times. This means, that all trips of a route and all routes of a costumer are in the same splitting. This has the advantage, that the cover constraints can be applied easily in the respective subproblems. The problem is that this formulation is potentially not equivalent to the original problem, \ie duties that are feasible in $\eqref{eq:MMILP}$ can be cut in this formulation. We show restrictions, in which the application of this splitting is sensible, though. 

\paragraph{Splitting} \parfill

The costumer-dependent splitting is defined as follows:
...
We denote the formulation $\eqref{eq:SMILP}$ with a splitting according to \Cref{def:costumer_dependent_splitting} as (CMILP).

\paragraph{Solving of the Partial Instances} \parfill

The formulation is built on the basic structure $\eqref{eq:SMILPi}$. The cover constraints are not considered so far. We therefore introduce the decision variable $u_m\in\{0,1\}$ for $m\in C^{-1}\left(\mathcal{C}_i\right)$. Since for a costumer $c\in\mathcal{C}_i$ all trips are in the splitting $\Ti$, only the cover constraints concerning these costumers are included in partial instance $I_i$. We therefore add the following constraints:
\begin{align}
...
Let $t_1$, $t_2$, $t_3$ with $t_1\prec t_2\prec t_3$ be trips with the properties shown in \Cref{tab:costumer_heuristic_example}
...
For the cover constraints, we again introduce the decision variable $u_m\in\{0,1\}$ for $m\in C^{-1}\left(\mathcal{C}_i\right)$.  Notice that the definition of $\mathcal{C}_i$ is different to the definition in \Cref{sec:costumer_dependent_splitting}. In the costumer constraints, only the costumers in this splitting are considered. The route constraints are restricted to the trips that are actually in this splitting. The cover constraints read as follows:
\begin{align}
...
For the constraint $\eqref{eq:TMILP:costumer}$ it is irrelevant, if the considered route has a trip in this splitting.
...
In order to choose a route in a partial instance, we have to estimate the costs for all routes of the same costumer in all subsequently solved partial instances in advance. The entire cost for the problem consists of vehicle costs $\cv$, trip costs $\ct$, deadhead costs $\cd$ and route costs $\croute$. While we can determine the trip costs and route costs easily for a route, the vehicle costs and trip costs strongly depend on the environment of the route and cannot be determined. We therefore focus on the trip and route costs and define the estimated route cost as follows:
\begin{align*}
...
We denote the formulation $\eqref{eq:SMILPi}$ with the constraints $\eqref{eq:TMILP:costumer}$, $\eqref{eq:TMILP:route}$, $\eqref{eq:TMILP:route_fix}$ and $\eqref{eq:TMILP:um}$ and the new objective function including modified route costs as $(\operatorname{TMILP}_i)$ for $i\in[n]$.
...
Given a solution of the problem, the subproblem is to find a costumer with a bad route choice. This means, for this costumer there is another route, such that the total cost is lower choosing this route. We can exchange these routes and compute a new solution considering the new route. 

An initial idea is to compute the cost, one route in the solution contributes to the entire solution. Then, we can compare this to the cost, with which we estimated the route costs before. If the actual cost are considerably higher than the estimated cost, this costumer is a candidate for exchanging routes. Since we cannot determine the contributing cost exactly, we try to estimate this cost.
...
With these definitions, we can adapt the formulation $(\operatorname{TMILP}_i)$ for $i=2$ to $(\operatorname{HSP}_c)$. The only modified constraints are the cover constraints $\eqref{eq:TMILP:costumer}$, $\eqref{eq:TMILP:route}$, $\eqref{eq:TMILP:um}$ and $\eqref{eq:TMILP:route_fix}$. They are replaced by
\begin{align}
...
We decide only the routes of costumer $c$. Thus, we use the objective function of $\eqref{eq:SMILPi}$ and add the following term:
\begin{align*}
...
We regard the special case in which each costumer has trips in at most two subsequent splittings. This can be ensured if the costumer extension is bounded by the splitting length. We try to exploit this special structure. For each costumer, we basically distinguish between two cases: There are more trips of this costumer in the splitting whose partial instance is solved first (Case 1) or there are more trips in the splitting whose partial instance is solved later (Case 2). In Case 1, the cost estimation for the routes is easy since most of the structure is already contained in the first processed partial instance. In Case 2, there is not much structure in the first processed partial instance, so the cost prediction will be imprecise. In order to prevent an imprecise cost estimation as in Case 2, we inspect the possibility of reversing a previous route choice in the later solved partial instance, if we find a better alternative there. In this section, we inspect the potential of cost saving for a belated trip deletion and develop a more flexible formulation in order to receive a better solution.
...
For every costumer $c\in\mathcal{C}$, there is $i\in[n-1]$ such that
\begin{align}
	t\in\left(\Ti\cupdot\mathcal{T}_{i+1}\right) && \text{for all } t\in (M\circ C)^{-1}(c)
\end{align}

this means, each costumer is represented in at most two subsequent splittings.

\end{lemma}

\begin{proof}

For simplicity of notion, we state $c_0:=-\infty$ and $c_n:=+\infty$. Consider costumer $c\in\mathcal{C}$ and $i\in[n]$ such that ${c_{i-1} \leq \zstart_c < c_i}$. For $i=n$ all trips of $c$ are in splitting $n$.	 For $i<n$ we have
\begin{align*}
...
In the following considerations, we neglect the costumer whose trip are in one splitting. These cover constraints are already ensured in the partial instance.

Consider partial instance $i\in[n]$, the costumer set
\begin{align*}
...
We want to regard the possibility of deleting an already chosen route in partial instance $I_i$. We therefore consider costumer $c\in\CR_i$, \ie the costumer has trips in an adjacent splitting and this partial instance is solved before. For this, we introduce the following notation.
...
The route $\bar{m}(c)$ denotes the route that is chosen for costumer $c\in\CR_i$ by the partial solution, $s_1(t)$ and $s_2(t)$ are the trips that are directly before and after this trip in its respective duty. 
...
In summary, the formulation $\eqref{eq:RTMILPi}$ is given by $\eqref{eq:SMILPi}$ with the constraints $\eqref{eq:TMILP:costumer}$, $\eqref{eq:RTMILP:costumer_old}$, $\eqref{eq:RTMILP:route_confirmed}$, $\eqref{eq:RTMILP:route_fix}$, $\eqref{eq:RTMILP:route}$, $\eqref{eq:RTMILP:um}$ and $\eqref{eq:RTMILP:uc}$ and the objective function
\begin{align}
...
It is possible that each costumer is represented in at most two splittings although $\eqref{eq:LCLS}$ does not hold. If this is not the case, one can possibly deviate some split points by a small value \st the condition holds. If only a small number of costumers exceeds the splitting length, their exclusion from the formulations $(\operatorname{HSP}_c)$ or $\eqref{eq:RTMILPi}$ still promises good results.
	\item{Create Preprocessing}:
One can create the split points via an own problem. The goal of this preprocessing is minimizing the costumers represented in several splittings, constraints are a minimal and a maximal splitting length. If there are only few costumers in more than one splitting, the solution behavior is improved.
