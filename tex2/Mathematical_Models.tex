...
We introduce the task graph, on which the model is based. This is a directed graph corresponding to the relation $\prec$ which we defined in \Cref{sec:problem_description}. The graph is basically the same as used in \cite{Kaiser_Knoll} with the only difference that the costumer and route considerations are adapted here.
