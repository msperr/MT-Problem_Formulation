...
Finally, we show that the original model and the time-dependent heuristic are equivalent to each other. We can transform each heuristical solution into a feasible solution of the $\eqref{eq:MMILP}$ as shown in \Cref{thm:equivalence_SMILP_MMILP} and \Cref{thm:equivalence_TMILP_SMILP}. In contrast to the customer-dependent, we additionally can transform each solution of the $\eqref{eq:MMILP}$ into a solution that is feasible in the heuristic as shown in \Cref{lem:equivalence_MMILP_SMILP} and \Cref{thm:equivalence_SMILP_TMILP}.
