...
Let $n\in\mathbb{N}$ and let \nomenclature[n]{$n$}{number of partial instances} \nomenclature[ti]{$\Ti$}{partial trip set}
\begin{align*}
...
\end{enumerate}
\nomenclature[sp]{$\SPjs$}{split point}
\nomenclature[pji]{$\mathcal{P}_{j,i}$}{partial split point set for trips in $\Ti$}
\nomenclature[pj]{$\mathcal{P}_j$}{partial split point set}
\nomenclature[p]{$\mathcal{P}$}{split point set}
...
\begin{enumerate}
	...
	\item{Transformed Task Graph: We define the transformed task graph $\overline{G}=\left(\overline{V},\overline{A}\right)$ with vertex set \nomenclature[ggraph]{$\overline{G}=\left(\overline{V},\overline{A}\right)$}{transformed task graph} \nomenclature[vgraph]{$\overline{V}$}{vertex set of the transformed task graph} \nomenclature[agraph]{$\overline{A}$}{arc set of the transformed task graph}
		\begin{align*}
...
\end{align}
\nomenclature[smilp]{$\left(\operatorname{SMILP}\right)$}{split arc flow formulation}
...
\begin{theorem}[Transformation of $\eqref{eq:SMILP}$ into $\eqref{eq:MMILP}$]
...
Note that a feasible solution in the $\eqref{eq:MMILP}$ is not necessarily feasible in the $\eqref{eq:SMILP}$. If there are ${s,t\in\mathcal{T}}$ with ${s\prec t}$ and ${s\in\mathcal{T}_{i+1}}, {t\in\Ti}$, then $s$ and $t$ cannot be connected in the transformed task graph. This issue is further discussed in \Cref{sec:customer_dependent_splitting}.
...
\nomenclature[i]{$I_i$}{partial instance}
\nomenclature[s]{$S_i$}{partial solution}
...
\nomenclature[phat]{$\Phat_i$}{set of endpoints}
...
\begin{algorithm}[htb]
	...
	\ForEach{$D_1\ni d=\left(s_1,\dots,s_l\right)$}{
		...
		\While{$t\in\bigcup_{i=1}^{n}\Phat_i$}{
			determine duty $d'=\left(s_{l+1},\dots,s_{l'}\right)$ with $s_{l+1}\in\mathcal{T}$ representing $t$\;
...
\end{definition}
\nomenclature[ggraph]{$\overline{G}_i=\left(\overline{V}_i,\overline{A}_i\right)$}{partial transformed task graph}
\nomenclature[vgraph]{$\overline{V}_i$}{vertex set of the partial transformed task graph}
\nomenclature[agraph]{$\overline{A}_i$}{arc set of the partial transformed task graph}
...
\end{align}
\nomenclature[smilp]{$\left(\operatorname{SMILP}_i\right)$}{basic formulation for a partial instance}
...
\begin{theorem}[Transformation of $\eqref{eq:SMILPi}$ into $\eqref{eq:SMILP}$]
...
Consider ${s\in\Phat_j}$ and ${x^j_{\ds,s}=1}$. Then the end point $s$ is not appended to a partial duty. In this case, \Cref{alg:successive_heuristic} creates a new end point in $\Phat_{j-1}$ out of $s$. If ${x^j_{\ds,s}=0}$, then we have ${x^j_{t',s}=1}$ for some ${t'\in\mathcal{T}_j}$ and therefore $s$ is appended to a duty and no further end point is created.
...
\begin{theorem}[Transformation of $\eqref{eq:SMILP}$ into $\eqref{eq:SMILPi}$]
...
\begin{align*}
	...
	\end{cases}
\end{align*}
\nomenclature[c]{$\mathcal{C}_i$}{partial customer set}
...
\nomenclature[cmilp]{$(\operatorname{CMILP}_i)$}{partial instance with customer-dependent splitting}
\nomenclature[cmilp]{$(\operatorname{CMILP})$}{split arc flow formulation with customer-dependent splitting}
...
\begin{theorem}[Transformation of $(\operatorname{CMILP})$ into $\eqref{eq:MMILP}$]
...
\end{definition}
\nomenclature[lc]{$L_{\operatorname{C}}$}{customer extension}
\nomenclature[ls]{$L_{\operatorname{S}}$}{splitting length}
...
\begin{align*}
	d_1(v) := \left(t_1,\dots,t_a\right) && d_2(v') := \left(t_{a+1}\right)
\end{align*}
...
\Cref{fig:doubleduties:duty} provides an example for a duty $d(v)$. The boxes indicate in which partial trip set the trips are located. \Cref{fig:doubleduties:d1d2} shows how the duties $d_1(v)$ and $d_2\left(v'\right)$ are constructed. In this example, it is crucial that $t_5$ is appended to $t_3$ although a connection to $t_4$ would also be feasible. If $t_5$ were appended to $t_4$ then there is no available duty for $t_7$.
...
Let $S=\left(x,z,e,u\right)$ be a feasible solution of the $\eqref{eq:MMILP}$. We prove that each duty $d$ of $S$ fulfills the conditions of \Cref{lem:doubleduties} or \Cref{rem:doubleduties}. Let ${v\in\mathcal{V}}$ be the vehicle that covers $d(v)$ and ${v'\in\mathcal{V}'}$ be the vehicle assigned to $v$.
...
Therefore we can apply \Cref{lem:doubleduties} or \Cref{rem:doubleduties} to each duty of $S$ individually. For each duty~$d(v)$, we receive duties $d_1(v)$ and $d_2(v')$, starting with vehicles $v$ and $v'$. If $d(v)$ is already feasible, the duty~$d_2(v')$ is empty. The duties $d_1(v)$ and $d_2(v')$ are feasible in the $(\operatorname{CMILP})$. We construct the new solution $S'$ with all new duties as constructed in \Cref{lem:doubleduties}. For this, we need the vehicles $v\in\mathcal{V}$ for the duties~$d_1(v)$ and the vehicles ${v'\in\mathcal{V}'}$ for the duties~$d_2(v')$. Since these duties are feasible in the heuristic, the time constraints and fuel constraints are not violated by $S'$. Since the set of covered trips has not changed during the process, that cover constraints are not violated, too. Therefore, the new solution~$S'$ is a feasible solution of the $(\operatorname{CMILP})$ and
\begin{align*}
	\operatorname{duties}\left(S'\right) \leq 2\cdot\operatorname{duties}\left(S\right).
\end{align*}
...
The fuel states are chosen such that all solutions are feasible \wrt fuel. We have ${v\prec t}$ for all ${t\in\mathcal{T}}$ and ${t_1\prec t_2}, {t_1\prec t_3}$. For customer~$b_1$ the trip~$t_1$ is fulfilled, for customer~$b_2$ either trip~$t_2$ or $t_3$. The optimal solution~$S^*$ consists of one duty $d^*(v)$ with
\begin{align*}
	d^*(v) = \left(t_1, t_2\right) && \operatorname{cost}\left(S^*\right) = 5.
\end{align*}

We solve this problem via \Cref{alg:successive_heuristic} with $n=2$ and $c_1=2$. Then the partial trip sets are ${\mathcal{T}_1=\left\{t_1\right\}}$ and ${\mathcal{T}_2=\left\{t_2,t_3\right\}}$. We first solve partial instance~$I_2$ where we chose the cheaper trip $t_3$ and receive ${\operatorname{cost}\left(S_2\right)=1}$. Therefore the heuristic solution~$S$ consists of duty $d(v)$ with
\begin{align*}
	d(v) = \left(t_1, t_3\right) && \operatorname{cost}\left(S\right) = K+3.
\end{align*}
...
\nomenclature[tmilp]{$(\operatorname{TMILP})$}{split arc flow formulation with time-dependent splitting}
...
Let ${i\in[n]}$. We define \nomenclature[ci]{$\overline{\mathcal{C}}_i$}{customers with fixed route choices}
\begin{align*}
...
\end{definition}
\nomenclature[uhat]{$\hat{u}^i_m$}{fixed route choice}
...
In summary, the objective function reads as: \nomenclature[tmilp]{$(\operatorname{TMILP}_i)$}{partial instance with time-dependent splitting}
\begin{align}
...
\begin{theorem}[Transformation of $\eqref{eq:TMILPi}$ into $\eqref{eq:SMILP}$]
...
\begin{theorem}[Transformation of $\eqref{eq:SMILP}$ into $\eqref{eq:TMILPi}$]
...
We further define \nomenclature[c2]{$C_2(S,m)$}{improved cost estimation}
\begin{align*}
...
\nomenclature[b]{$B$}{set of reviewed customers}
...
\nomenclature[bj]{$B_j$}{set of reviewed customers in an iteration}
...
\end{itemize}
\nomenclature[rmin]{$r_{\min}$}{minimal ratio in the iterative heuristic}
\nomenclature[cmax]{$c_{\max}$}{maximal number of customers in the iterative heuristic}
\nomenclature[tmax]{$t_{\max}$}{maximal time range in the iterative heuristic}
\nomenclature[nmax]{$n_{\max}$}{maximal number of iterations in the iterative heuristic}
...
\nomenclature[hsp]{$(\operatorname{HSP}_B)$}{subproblem of the iterative heuristic}
\nomenclature[vhat]{$\Vhat$}{set of start points}
...
\end{align}
\nomenclature[fmilp]{$(\operatorname{FMILP}^{\min}_S)$}{formulation to determine minimal fuel states}
\nomenclature[fmilp]{$(\operatorname{FMILP}^{\max}_S)$}{formulation to determine maximal fuel states}
...
\end{enumerate}
\nomenclature[s]{$S^i$}{improved solution in the iterative heuristic}
...
Analogously to the customer-dependent heuristic, the time-dependent heuristic is not a constant-factor approximation. \Cref{ex:approximation_factor} directly applies to the $(\operatorname{TMILP})$, too. With the same choice of the time point~$c_1$, the splitting $\left\{\mathcal{T}_1,\mathcal{T}_2\right\}$ is equal if it is performed according to \Cref{def:time_dependent_splitting}. Thus the heuristic solution can be arbitrarily bad compared to the optimal solution, even if all subproblems are solved to optimality.
...
We use the heuristic solution in order to provide an initial solution for the Optimal Approach. This is examined in \Cref{ch:optimal_approach}.